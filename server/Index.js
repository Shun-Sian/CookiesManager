...
app.post('/update-preference/:id', async (req, res) => {
  ...
  const preferenceToUpdate = preferences.find(preference => preference.id === preferenceId)
  ...
  return res.status(201).json({ message: 'Successfully updated preference', preference: preferenceToUpdate });
});
